...
# define function to extract numbers from raw instruction
parse_instr <- function(instr) {
  return(
    as.numeric(
      str_extract(
        instr,
        "move ([0-9][0-9]*) from ([0-9][0-9]*) to ([0-9][0-9]*)",
        group = c(1, 2, 3))
    )
  )
}
...
stacks.list <- list()
for (s in raw_stacks) {
  stacks.list <- append(stacks.list, list(word2vec(s)))
}
...
stacks <- stacks.matrix %>% as.data.frame() %>% as.list()
...
Can be solved the same way but without reversing the crates being moved
...
for (i in 1:nrow(instr)) {
  ...
  to <- instr[i,]$To
  from_height <- length(stacks[[from]])
  moving_crates <- stacks[[from]][(from_height - n_crates + 1):from_height]
  stacks[[from]] <- stacks[[from]][1:(from_height - n_crates)]
  stacks[[to]] <- append(stacks[[to]], moving_crates)
}
